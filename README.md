...
| `stock_movements`, `stock_alerts`, `stock_low_products` | `GET /stock-movements`, `/alerts`, `/stock/low-products` |
...
`stock_low_products` interroge directement l'état courant du catalogue avec la même
règle que le compteur BATIX PRO. La réponse est paginée et `meta.total` fournit le total
exact, indépendamment de l'historique des alertes lues ou non lues.
